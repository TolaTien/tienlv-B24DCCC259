...
const INITIAL_DESTINATIONS: TravelPlan.Destination[] = [
  {
    id: '1',
    name: 'Vịnh Hạ Long',
...
export const TravelService = {
  getDestinations: (): TravelPlan.Destination[] => {
  ...
  saveDestinations: (destinations: TravelPlan.Destination[]) => {
    localStorage.setItem(DESTINATIONS_KEY, JSON.stringify(destinations));
  },

  addDestination: (destination: TravelPlan.Destination) => {
    const destinations = TravelService.getDestinations();
    destinations.push(destination);
    TravelService.saveDestinations(destinations);
  },

  updateDestination: (destination: TravelPlan.Destination) => {
    const destinations = TravelService.getDestinations();
  ...
  deleteDestination: (id: string) => {
    ...
    TravelService.saveDestinations(filtered);
  },

  getItineraries: (): TravelPlan.Itinerary[] => {
    const data = localStorage.getItem(ITINERARIES_KEY);
    return data ? JSON.parse(data) : [];
  },

  saveItineraries: (itineraries: TravelPlan.Itinerary[]) => {
    localStorage.setItem(ITINERARIES_KEY, JSON.stringify(itineraries));
  },

  addItinerary: (itinerary: TravelPlan.Itinerary) => {
    const itineraries = TravelService.getItineraries();
    itineraries.push(itinerary);
    TravelService.saveItineraries(itineraries);
  },
};
