    ...
    <div style={{ padding: 24, background: '#fff', minHeight: 360 }}>
      ...
      <Card title="Thiết lập cấu trúc đề" style={{ marginBottom: 24 }}>
        <Form form={form} layout="vertical" onFinish={handleGenerateExam}>
          ...
          <div style={{ marginBottom: 16 }}><b>Cấu trúc câu hỏi:</b></div>
          <Form.List name="structure" initialValue={[{}]}>
            {(fields, { add, remove }) => (
              <>
                {fields.map(({ key, name, ...restField }) => (
                  <Space key={key} style={{ display: 'flex', marginBottom: 8 }} align="baseline">
                    <Form.Item {...restField} name={[name, 'knowledgeBlockId']} rules={[{ required: true, message: 'Chọn khối KT' }]}>
                      <Select placeholder="Khối kiến thức" style={{ width: 180 }}>
                        {knowledgeBlocks.map(k => <Option key={k.id} value={k.id}>{k.name}</Option>)}
                      </Select>
                    </Form.Item>
                    <Form.Item {...restField} name={[name, 'difficulty']} rules={[{ required: true, message: 'Chọn mức độ' }]}>
                ...
                <Form.Item><Button type="dashed" onClick={() => add()} block icon={<PlusOutlined />}>Thêm tiêu chí</Button></Form.Item>
              </>
            )}
          </Form.List>
          <Form.Item><Button type="primary" htmlType="submit" size="large">Sinh đề thi tự động</Button></Form.Item>
        </Form>
      </Card>
      ...
      <Table 
        ...
        columns={[
          ...
          { title: 'Hành động', render: (_, r) => <Button type="link" onClick={() => setViewExamModal(r)}>Xem chi tiết</Button>}
        ]} 
      />
