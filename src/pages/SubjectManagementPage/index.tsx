  ...
  return (
    <div style={{ padding: 24, background: '#fff', minHeight: 360 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        <Button type="primary" onClick={() => { form.resetFields(); setIsModalVisible(true); }}>+ Thêm môn học</Button>
      </div>
      <Table columns={columns} dataSource={subjects} rowKey="id" bordered />
      <Modal title="Thông tin môn học" visible={isModalVisible} onCancel={() => setIsModalVisible(false)} onOk={() => form.submit()}>
        <Form form={form} layout="vertical" onFinish={handleSave}>
          <Form.Item name="id" hidden><Input /></Form.Item>
          <Form.Item name="code" label="Mã môn học" rules={[{ required: true }]}><Input placeholder="VD: INT3306" /></Form.Item>
